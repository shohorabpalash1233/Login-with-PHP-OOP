...
class LoginRegistration {
    ...
    public function registerUser($username, $password, $name, $email, $website) {
        ...
        if ($num == 0) {
            ...
            return print "<span style='color: #e53d37'>Username/Email already used</span>";
        }
    }
    ...
    public function updatePassword($uid, $new_pass, $old_pass) {
        ...
        $num = $query->rowCount();
        if ($num == 0) {
            return print("<span style='color: #e53d37;'>Old Password not exists</span>");
        } else {
            $query = $pdo->prepare("UPDATE users SET password = ? WHERE id = ?");
            $query->execute(array($new_pass, $uid));
            print("<span style='color: green'>Password changed successfully.</span>");
        }
    }
